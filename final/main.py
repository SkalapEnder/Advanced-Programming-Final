...
# --- Initialize Search & Embedding System ---
...
def search_query_internet(query):
    """Performs a Google Search and fetches content from web pages."""
    urls = []
    try:
    ...
    print(f"Google Search URLs: {urls}")  # Debugging
...
def truncate_text(text):
    """Limits the extracted text to a reasonable length."""
    words = text.split()
...
with st.form("prompt_form"):
    ...
    number_images = st.number_input("Select number of images to analyze:", min_value=3, max_value=10, value=3)
